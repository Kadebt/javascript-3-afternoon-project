...
//Code Here
let evenNumbers = mixedNumbers.filter((num) => num % 2 != 1)
...
//Code Here
function newMonsters(monsters){
  if(monsters.CP > 200){
    return true
  }
}
  
let myStrongest = monstersInYourPocket.filter(newMonsters)
...
//   let taxes = orders.tax + 1
//   let totalPrice = orders.price * taxes
//   return totalPrice
// }
// let totalprice = orders.map(orderTotals)
// console.log(totalprice)

let orderTotals = orders.map(e => {
  const taxRate = e.tax + 1
  const total = e.price * taxRate
  return total
})
...
var bobsTotal = purchases.reduce((total, price) => {
    if(price['owner'] === 'Bob'){
       total += price['price'];
      }
     return total;
